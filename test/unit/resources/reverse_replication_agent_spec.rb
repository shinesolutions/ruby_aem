    ...
    it 'should call client with expected parameters having custom optional parameters' do
      expect(@mock_client).to receive(:call).once().with(
        ...
        { :run_mode => 'author',
          ...
          :log_level => 'info',
          :retry_delay => 60000 })
      @reverse_replication_agent.create_update('Some Reverse Replication Agent Title', 'Some reverse replication agent description', 'http://somehost:8080', { transport_user: 'someuser', transport_password: 'somepassword', log_level: 'info', retry_delay: 60000 })
    end
